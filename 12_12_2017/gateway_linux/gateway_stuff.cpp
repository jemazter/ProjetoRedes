...
Transceiver::Transceiver(){
  ...
  //RF24 radio(22,0);
  gatewayAcess=0;
  myPhysicalAddr[5] = 0x00;
  int i;
  for(i=0;i<5;i++)
  	myPhysicalAddr[i] = 0x00;    
  myDynamicAddr = GATEWAY_ADDR;
  myPackageId = 0;
}
    ...
    switch( op ) {
      case PAYLOAD_SEND:
        if ( dataReceived.destino == myDynamicAddr ) {
          #ifdef debug
          printf("RECEBENDO: ORIGEM: %X DESTINO: %X HOP: %d OPCODE: %X ID: %X CHKSUM: %X\n",dataReceived.origem, dataReceived.destino, dataReceived.hop, dataReceived.opcode, dataReceived.id, dataReceived.checksum);
	  for(i; i<4; i++)
		printf("%d",dataReceived.payload[i]);
	  printf("\n");
	  #endif
        }
        else {
